...
def trainbody():
    ...
    joblib.dump(model,'./model/dancehandGBDT.pkl')
...
def predictframe(model):
    ...
    for index, row in df.iterrows():
        ...
        if framename=='video':
            ...
            tagdict[row[0].split('_')[0]+'_'+row[0].split('_')[1]] = preres[0]
    ...
    fileObject = open(r'video1handdict.json', 'w')
...
#给图片打标签
def brandtag():
    jsdict = open("video1handdict.json", encoding='utf-8')
    jsdict2 =  open("video1dict.json", encoding='utf-8')
    mydict1 = dict(json.load(jsdict))
    mydict2 = dict(json.load(jsdict2))
    mydict1.update(mydict2)
    imgdir = r'D:\big3data\down\wulian\mydataset\testdance\video1res\videoimages'
    keyframeforvideo = set(mydict1.keys())
    print('keys:')
    ...
    for each in imgnames:
        ...
        if tag in keyframeforvideo:
            ...
            bk_img = cv2.putText(bk_img, 'pose: '+mydict1[tag], (10, 30), cv2.FONT_HERSHEY_SIMPLEX,
                    0.8, (0, 0, 255), 1, cv2.LINE_AA)
            # cv2.imshow('haha.jpg',bk_img)
            tardir = r'D:\big3data\down\wulian\toUnity\imgwithtag'
            cv2.imwrite(tardir+'/'+each,bk_img)
            # cv2.waitKey(0)
        else:
            bk_img = cv2.imread(imgdir + '/' + each)
            bk_img = cv2.resize(bk_img, (432, 768), interpolation=cv2.INTER_CUBIC)
            bk_img = cv2.putText(bk_img, 'pose: ', (10, 30), cv2.FONT_HERSHEY_SIMPLEX,
                                 0.8, (0, 0, 255), 1, cv2.LINE_AA)
            # cv2.imshow('haha.jpg', bk_img)
            tardir = r'D:\big3data\down\wulian\toUnity\imgwithtag'
            cv2.imwrite(tardir + '/' + each , bk_img)
if __name__=="__main__":
    df = pd.read_csv('./data/dancedata/handres.csv')
    # print(df.info())
    Y = df.iloc[:,-1]
    print(Y)
    X = df.iloc[:, 1:-2]
